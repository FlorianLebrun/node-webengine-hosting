...
void WebxEngineJS::New(const Nan::FunctionCallbackInfo<v8::Value> &args)
{
  ...
  Local<String> dllPath = args[0]->ToString(Isolate::GetCurrent());
  Local<String> dllEntry = args[1]->ToString(Isolate::GetCurrent());
  Local<String> config = args[2]->ToString(Isolate::GetCurrent());

  WebxEngine *engine = new WebxEngine(args[3].As<v8::Function>());
  engine->AttachObject(args.This());
  engine->connect(*Utf8Value(Isolate::GetCurrent(),dllPath), *Utf8Value(Isolate::GetCurrent(),dllEntry), *Utf8Value(Isolate::GetCurrent(),config));
...
v8::Local<v8::Function> WebxEngineJS::CreatePrototype()
{
  ...
  auto tpl_function = tpl->GetFunction(v8::Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked();
  constructor.Reset(tpl_function);
  return tpl_function;
}
