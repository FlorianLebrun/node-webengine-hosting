...
std::string resolve_with_env_var(std::string value) {
  char buffer[2048];
  if (ExpandEnvironmentStringsA(value.c_str(), buffer, sizeof(buffer))) {
...
std::map<std::string, std::string> parse_arguments(const char** argv, int argc) {
  ...
  for (int i = 0; i < argc; i++) {
    if (argv[i][0] == '-') {
      if (++i < argc) args[argv[i - 1]] = resolve_with_env_var(argv[i]);
    }
  }
  return args;
}
...
int main(int argc, const char** argv) {
  ...
  if (!GetCurrentDirectoryA(sizeof(currentDirectory), currentDirectory)) {
    printError("Cannot read current directory");
    return -1;
  }

  // Read configuration file
  printf("Load configuration at '%s'\n", args["--config"].c_str());
  printf("> with current directory '%s'\n", currentDirectory);
  HANDLE hFile = CreateFile(args["--config"].c_str(), GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  if (hFile == INVALID_HANDLE_VALUE) {
    printError("Web agent configuration file not found (see '--config')");
    return -1;
  }
  ...
  if (!hAgentDll) {
    printError("Web agent dll cannot be loaded (see '--dll-path')");
    return -1;
  }
  ...
  if (!agentConnect) {
    printError("Web agent dll entrypoint not found (see '--dll-entrypoint')");
    return -1;
  }
  ...
  if (!context) {
    printError("Web agent connect procedure has failed");
    return -1;
  }

  // Run server
  std::string url = "http://127.0.0.1:" + args["--port"] + "/";
  if (server.Init(url, context)) {
    server.Loop();
    return 0;
  }
  else {
    printError("Server cannot start at '%s' (see '--port')", url.c_str());
    return -1;
  }
}
