  ...
  function parseFile(file) {
    ...
    code.split(/\r?\n/g).forEach(function (line) {
      ...
      if ((line.indexOf("from") >= 0 && (match = line.match(importEs6Exp))) ||
        ...
        assert(moduleName);
        if (moduleName[0] !== '.') {
          var alias_impPath = path.resolve(baseDir, "./" + moduleName);
          if (fs.existsSync(alias_impPath) || fs.existsSync(alias_impPath + '.d.ts')) {
            moduleName = path.relative(path.dirname(file), alias_impPath).replace(/\\/g, "/")
        ...
        if (inSourceTypings(file)) {
          if (line.indexOf("import") < 0) res.lines.push({ original: line.replace(/^(export )?declare /g, '$1') });
        }
        else {
          res.lines.push({ original: line });
        }
