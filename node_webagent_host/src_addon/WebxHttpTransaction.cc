  ...
  : responseData(this, this->completeEvents_sync)
{
  ...
  // Set request headers
  Local<Object> headers = v8h::GetIn(req, "headers")->ToObject(v8::Isolate::GetCurrent());
...
void WebxHttpTransaction::completeEvents() {
  ...
  if (this->response) {
    ...
    onSend->Call(isolate->GetCurrentContext(), v8::Null(v8::Isolate::GetCurrent()), 3, argv);
    this->response = 0;
  }
  else {
    Local<Value> argv[] = {
    ...
    onSend->Call(isolate->GetCurrentContext(), v8::Null(v8::Isolate::GetCurrent()), 2, argv);
  }

  // Close the response stream
  if (this->responseData.is_completed()) {
    Local<Function> onEnd = Local<Function>::New(isolate, this->onEndCallback);
    onEnd->Call(isolate->GetCurrentContext(), v8::Null(v8::Isolate::GetCurrent()), 0, 0);
    this->DettachObject();
    this->release();
  }
}
