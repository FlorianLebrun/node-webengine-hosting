...
void WebxSessionJS::New(const Nan::FunctionCallbackInfo<v8::Value> &args)
{
  ...
  if (WebxEngine *engine = WebxEngine::Unwrap<WebxEngine>(args[0]->ToObject(v8::Isolate::GetCurrent()))) {
    std::string type = *Utf8Value(v8::Isolate::GetCurrent(), args[1].As<v8::String>());
    std::string name = *Utf8Value(v8::Isolate::GetCurrent(), args[2].As<v8::String>());
